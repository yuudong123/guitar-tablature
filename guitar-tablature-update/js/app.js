$(document).ready(function () {
  ...
  Buttons.on("click", (event) => {
    ...
    switch (cls) {
      ...
      // }
      case "add-line-btn": {
        const chordTable = $("#chord-table tbody");
        const newChordLine = $("<tr class='chord-line'></tr>");

        for (let i = 0; i < 8; i++) {
          const newChordInput = $(
            "<td><input type='text' class='chord-input'/></td>"
          );
          newChordLine.append(newChordInput);
        }

        chordTable.append(newChordLine);
        cntLine++;
        Lines.text(cntLine);
        break;
      }
